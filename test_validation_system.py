...
class TestValidator:
    ...
    def analyze_test_file(self, file_path: str) -> TestAnalysis:
        ...
        for pattern in self.critical_patterns:
            if re.search(pattern, content, re.MULTILINE):
                issues.append(f"Critical pattern found: {pattern}")

        # Check for warning patterns
        for pattern in self.warning_patterns:
            if re.search(pattern, content, re.MULTILINE):
                issues.append(f"Warning pattern found: {pattern}")
        ...
        if not any('unittest' in imp or 'pytest' in imp for imp in imports):
        ...
        quality_score = self._calculate_quality_score(len(test_methods), len(issues), len(recommendations))
    ...
    def _calculate_quality_score(self, test_count: int, issue_count: int, rec_count: int) -> float:
        ...
        rec_bonus = min(0.2, rec_count * 0.05)

        return max(0.0, min(1.0, base_score - issue_penalty + rec_bonus))
    ...
    def generate_report(self, report: ValidationReport) -> str:
        ...
        lines.append(f"Autopilot Ready: {'✅ Yes' if report.autopilot_ready else '❌ No'}")
        lines.append("")
...
class AutopilotSystem:
    ...
    def run_autopilot(self, dry_run: bool = False, parallel: bool = False) -> Dict[str, Any]:
        ...
        print("🚀 Starting Autopilot Test System")
        ...
        if not report.autopilot_ready and not dry_run:
            print("❌ Tests not ready for autopilot. Run with --dry-run to see issues.")
            return {
        ...
        if dry_run:
            print("🏃 Phase 3: Dry Run (Validation Only)")
            return {
