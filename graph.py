    ...
    for i in range(n_entities - 1):
        for j in range(i + 1, min(i + 4, n_entities)):
            if np.random.random() > 0.5:
                G.add_edge(
                    f"entity_{i}", f"entity_{j}", relationship="knows", strength=np.random.random()
                )
...
def create_timeline_graph(
    start_date: datetime, end_date: datetime, resolution: str = "day"
) -> nx.DiGraph:
    ...
    while current <= end_date:
        ...
        G.add_node(
            timepoint_id, timestamp=current, resolution=resolution, training_status="untrained"
        )
