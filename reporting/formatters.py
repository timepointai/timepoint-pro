...
class FormatterFactory:
    ...
    def create(cls, format: str, **kwargs) -> OutputFormatter:
        ...
        if format_lower not in cls._formatters:
            raise ValueError(
                f"Unsupported format: {format}. Supported: {list(cls._formatters.keys())}"
            )
...
class FountainFormatter:
    ...
    def _format_title_page(self, script_data) -> str:
        ...
        if script_data.metadata:
            ...
            lines.append(
                f"Scenes: {script_data.metadata.get('total_scenes', len(script_data.scenes))}"
            )
            lines.append(
                f"Characters: {script_data.metadata.get('entities_count', len(script_data.characters))}"
            )
