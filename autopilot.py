...
class EnhancedAutopilot:
    ...
    def run_full_autopilot(self, dry_run: bool = False, parallel: bool = True,
        ...
        print("🚀 Timepoint-Daedalus Autopilot System")
        print("=" * 50)
        ...
        if not report.autopilot_ready and not dry_run and not self.config.get('force_execution', False):
            print("❌ Tests not ready for autopilot. Critical issues found.")
        ...
        if not validated_tests and not dry_run:
            print("❌ No validated tests available for execution.")
        ...
        if critical_issues_in_validated and not dry_run:
            print(f"❌ {len(critical_issues_in_validated)} critical issues found in validated tests.")
        ...
        # Phase 3: Execution
        print(f"\n🏃 Phase 3: Test Execution ({len(validated_tests)} test files)")
    ...
    def _filter_validated_tests(self, report: Any) -> List[str]:
        ...
        print(f"✅ Filtered {len(validated)} high-quality test files from {len(self.test_files)} total")
        return validated
    ...
    def _run_parallel_execution(self, test_files: List[str], max_workers: int) -> Dict[str, Any]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(futures):
                ...
                print(f"  ✓ Completed {test_file} ({result['passed']}/{result['total']} passed)")
    ...
    def _print_summary(self, results: Dict[str, Any]):
        ...
        if analysis.get('failed_tests'):
            ...
            for failed in analysis['failed_tests'][:5]:  # Show first 5
                print(f"  • {failed['file']}: {failed['failed']}/{failed['total']} failed")
