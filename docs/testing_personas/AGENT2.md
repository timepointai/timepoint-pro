...
**What first attracted him:** The Castaway Colony template. He read the description --- "Six crew members crash-land on Kepler-442b and must choose between three survival strategies" with deterministic resource propagation via the QuantitativeStateEngine --- and his immediate reaction was: this is a simplified version of the problem I work on every day. He cloned the repo the same evening and ran the template. The quantitative state propagation (resource tracking per entity per timepoint) and the knowledge provenance system (who knows about the hull breach, when they learn about it, how that information propagates to the commander's decision-making) were directly relevant to his mission assurance work.

The constraint enforcement mechanism (M4) was what moved him from "interesting academic exercise" to "I need to evaluate this seriously." The idea that resource constraints are enforced structurally rather than as post-hoc checks maps to how he thinks about failure propagation: you cannot violate mass conservation in a propulsion failure scenario, and you should not be able to violate information conservation in a crew decision-making scenario.
