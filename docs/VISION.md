...
In a standard transformer, attention weights perform a soft version of this allocation within a fixed context window. But the allocation is recomputed from scratch at every layer, with no persistent model of which entities deserve attention over time. The fidelity surface in Timepoint Daedalus is the explicit, persistent version: most entities sit at TENSOR resolution (~200 tokens) indefinitely, and the system only pays for full resolution (~50,000 tokens) when a query demands it. The result is a 95% token reduction without loss of causal structure.
