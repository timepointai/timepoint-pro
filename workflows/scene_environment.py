...
from schemas import AtmosphereEntity, CrowdEntity, Entity, EnvironmentEntity
...
def compute_scene_atmosphere(
    ...
    if llm_client is not None and timepoint_info is not None:
        try:
            ...
            entity_dicts = [
                {"entity_id": e.entity_id, "entity_type": e.entity_type} for e in entities[:20]
            ]
...
def compute_crowd_dynamics(entities: list[Entity], environment: EnvironmentEntity) -> CrowdEntity:
    ...
    for entity in entities:
        try:
            cognitive = entity.cognitive_tensor
            mood = classify_emotional_state(
                cognitive.emotional_valence, cognitive.emotional_arousal
            )
            mood_counts[mood] += 1
            total_entities += 1
        except (AttributeError, KeyError):
...
def infer_formality_from_location(location: str) -> float:
    ...
    # Medium formality
        ...
        term in location_lower for term in ["meeting house", "assembly", "congress", "senate"]
    ):
        return 0.7
